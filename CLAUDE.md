...
- Rescheduling recomputes the invoice. If the new total exceeds what was already paid, the booking moves immediately but `invoices.chenh_lech_cho_tt` records the balance the customer must pay (payment page runs in "difference" mode; email is sent). If the new total is lower or equal, nothing more is due and the overpayment is not refunded. `POST /api/bookings/{id}/reschedule-preview` returns the bill without changing anything.
